...
email_out=/tmp/ACDCBestPrice_out.txt
...
if [ $PSS_ENV = "TEST" ]
    ...
    export scm_sql=/data/scm/sql
    export SLICSRV=stl_dprdsup
    export scm_exe=/mcair/dev/appl/scm/work/c
else
...
more $scm_sql/ACDCBestPrice_get_parts.sql| 
...
if [ $rc -eq 0 ]
  then
	...
	$scm_exe/ACDCBestPrice/ACDCBestPrice -a$ACDC_EXTRACTED_PARTS -b$ACDC_EXTRACTED_BEST_PRICE_PARTS -c$FISCAL_PERIOD>>$email_out
	rc=$?
	if [ $rc -eq 0 ]
	  then
...
if [ $rc -eq 0 ]
  then
	...
	echo "#----------------------------------------------------------------------------------------#"
	ACDC_EXTRACTED_NO_ORDER_PARTS=$dta_scm/ACDCNoOrder_Parts.txt
	...
	$scm_exe/ACDCNoOrderParts/ACDCNoOrderParts -a$ACDC_EXTRACTED_NO_ORDER_PARTS -b$FISCAL_PERIOD -c$PROCESSING_MDL>>$email_out
	rc=$?
	if [ $rc -eq 0 ]
	  then
...
if [ $rc -eq 0 ]
  then
	...
	echo "#----------------------------------------------------------------------------------------#"
	ACDC_CONSOLIDATED_BEST_PRICE_PARTS=$dta_scm/ACDCConsolidate_Priced_Parts.txt
...
#	CTLFILE=$scm_home/load/ACDC_Best_Price_new.ctl;export CTLFILE
	CTLFILE=/data/scm/load/ACDC_Best_Price_new.ctl;export CTLFILE
	...
	if [ -f $DATAFILE ]
  	 then
	     ...
	     export PSS_ORACLE_SID=$PSS_ORACLE_SID
             ...
             . /home/oracle/scripts/oracle_db"$PSS_VRSN"_setup.sh "$PSS_ORACLE_SID";
...
if [ $rc -eq 0 ]
  then
	...
	CONNECT=$SLICSRV
	ACDC_EXTRACTED_OTHER_PARTS=$dta_scm/ACDC_Extracted_Other_Parts.txt
	...
	more $scm_sql/ACDCOtherParts.sql| 
			sed 's/MYSCHEMA/'$SCHEMA'/g'|
			sed 's/ACCESS/'$ACCESS'/g'|
			sed 's/CONNECT/'$CONNECT'/g'>$SQLFILE
...
if [ $rc -eq 0 ]
  then
	...
	echo "#----------------------------------------------------------------------------------------#"
	ACDC_EXTRACTED_OTHER_SLIC_PRICED_PARTS=$dta_scm/ACDC_Extracted_Other_Parts_SLIC.txt
	...
	$scm_exe/ACDCOtherParts/ACDCOtherParts -a$ACDC_EXTRACTED_OTHER_PARTS_SORTED -b$ACDC_EXTRACTED_OTHER_SLIC_PRICED_PARTS -c$FISCAL_PERIOD -d$PROCESSING_MDL>>$email_out
	rc=$?
	if [ $rc -eq 0 ]
	  then
...
if [ $rc -eq 0 ]
  then
	...
	echo "#----------------------------------------------------------------------------------------#"
	ACDC_CONSOLIDATED_ALL_PARTS=$dta_scm/ACDCConsolidate_ALL_Priced_Parts.txt
	echo "Input Extracted ACDC No Order Parts "$ACDC_EXTRACTED_NO_ORDER_PARTS
	echo "Input Extracted ACDC No Order Parts "$ACDC_EXTRACTED_NO_ORDER_PARTS>>$email_out
	echo "Input Extracted ACDC Best Price Parts "$ACDC_EXTRACTED_BEST_PRICE_PARTS
	echo "Input Extracted ACDC Best Price Parts "$ACDC_EXTRACTED_BEST_PRICE_PARTS>>$email_out
	echo "Input Extracted Other ACDC Parts Sorted "$ACDC_EXTRACTED_OTHER_PARTS_SORTED
	echo "INput Extracted Other ACDC Parts Sorted "$ACDC_EXTRACTED_OTHER_PARTS_SORTED>>$email_out
...
if [ $rc -eq 0 ]
    then
	more $email_out|mailx -m -s"Successful Completion ACDC Load Best Price Table" larry.d.mills@boeing.com
else
	more $email_out|mailx -m -s"Abnormal Termination ACDC Load Best Price Table" larry.d.mills@boeing.com
fi
exit $rc
